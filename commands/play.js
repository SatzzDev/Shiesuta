const { card, trackBody } = require("../lib/ui");
...
module.exports = {
    ...
    async run(m, args, { kazagumo, t }) {
        ...
        if (!channel) return m.reply(card({ title: `## ${EMOJI.error} Error`, body: t.noVoice, color: 0xED4245 }));
        const query = args.join(" ");
        if (!query) return m.reply(card({ title: `## ${EMOJI.error} Error`, body: t.noQuery, color: 0xED4245 }));
        ...
        if (!result.tracks.length) return m.reply(card({ title: `## ${EMOJI.error} Error`, body: t.noResults, color: 0xED4245 }));
